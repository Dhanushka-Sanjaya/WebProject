...
add_action( 'widgets_init', 'elletta_latest_posts_category_load_widget' );

function elletta_latest_posts_category_load_widget() {
	register_widget( 'elletta_latest_posts_category_widget' );
}

class elletta_latest_posts_category_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'elletta_latest_posts_category_widget', // Base ID
			__( 'elletta - Category Posts with Image', 'elletta' ), // Name
			array(
				'description' => esc_html__( 'A widget that displays a category posts with image', 'elletta' ), 
				'classname' => 'elletta_latest_posts_category_widget',
				'width' => 250,
                                'height' => 350
			) 
		);
					...
					<div class="side-item">
						...
						<div class="side-image">
							<a href="<?php echo esc_url(get_permalink()); ?>" rel="bookmark" title="<?php the_title(); ?>">
								<?php the_post_thumbnail('elletta_thumbnail-featured-3', array('class' => 'side-item-thumb')); ?>
	...
	public function form( $instance ) {
		...
		<!-- Category -->
		<p>
		<label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e('Filter by Category:', 'elletta'); ?></label> 
		<select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
			<option value='' <?php if ('' == $instance['categories']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'elletta'); ?></option>
		...
		<!-- Number of posts -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e('Number of posts to show:', 'elletta'); ?></label>
